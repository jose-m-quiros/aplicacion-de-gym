...
async function saveExercise(day, exerciseName, fieldPrefix) {
    ...
    rows.forEach((row, index) => {
        ...
        const partialInput = document.getElementById(`${fieldPrefix}_partial${serieNum}`);
    ...
    try {
        ...
        if (result.status === 'success') {
            showMessage(`✅ ${exerciseName} guardado (${seriesData.length} series)`, 'success');
            
            // Limpiar los campos
            card.querySelectorAll('input[type="number"], input[type="text"]').forEach(input => {
